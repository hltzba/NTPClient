    ...
    public class SNTPClient
    {
        ...
        public void Connect(bool UpdateSystemTime)
        {
            try
            {
                ...
                TimeSocket.Client.ReceiveTimeout = 25000;
        ...
        public override string ToString()
        {
            ...
            switch (LeapIndicator)
            {
                ...
                case _LeapIndicator.LastMinute61:
                    str += "最后一分钟包含了异常的61秒";//"Last minute has 61 seconds";
                    break;

                case _LeapIndicator.LastMinute59:
                    str += "最后一分钟只获得59秒（异常）";//"Last minute has 59 seconds";
                    break;

                case _LeapIndicator.Alarm:
                    str += "同步失败";//"Alarm Condition (clock not synchronized)";
                    break;
            }
            str += "\r\nNTP版本：" + VersionNumber.ToString() + "\r\n";//"\r\nVersion number: " + VersionNumber.ToString() + "\r\n";
            str += "远程模式：";//"Mode: ";
            switch (Mode)
            {
                case _Mode.Unknown:
                ...
                case _Mode.SymmetricActive:
                    str += "主动对称";//"Symmetric Active";
                    break;

                case _Mode.SymmetricPassive:
                    str += "被动对称";//"Symmetric Pasive";
                    break;
